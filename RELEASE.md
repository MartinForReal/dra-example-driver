...
- The dra-infiniband-driver Helm chart
- Container images
...
1. An issue proposing a new release is opened.
   ...
   chart should be released.
     - Example issue: https://github.com/kubernetes-sigs/dra-infiniband-driver/issues/119
...
1. When releasing new container images, the Helm chart's `appVersion` in
   Chart.yaml is updated to reflect the version of the images to be cut.
    - Example PR: https://github.com/kubernetes-sigs/dra-infiniband-driver/pull/120
1. Depending on what is being released, an OWNER runs `git tag -a $VERSION -m $VERSION`
   and pushes the tag with `git push <remote> $VERSION`
    ...
    - `<remote>` refers to the name of the Git remote for the repo at
      [https://github.com/kubernetes-sigs/dra-infiniband-driver], likely either
      `upstream` or `origin`
    - Each pushed tag triggers builds of artifacts pushed to the staging
      repository for the [dra-infiniband-driver container image][container-staging]
...
1. The artifacts are verified to be available from registry.k8s.io:
    - e.g. the v0.2.0 image and 0.2.0 chart can be verified with the following
      commands:
        - `docker manifest inspect registry.k8s.io/dra-infiniband-driver/dra-infiniband-driver:v0.2.0`
        - `docker manifest inspect registry.k8s.io/dra-infiniband-driver/charts/dra-infiniband-driver:0.2.0`
...
[staging repo]: https://console.cloud.google.com/artifacts/docker/k8s-staging-images/us-central1/dra-infiniband-driver?inv=1&invt=Abs5-A&project=k8s-staging-images
[chart-staging]: https://console.cloud.google.com/artifacts/docker/k8s-staging-images/us-central1/dra-infiniband-driver/charts%2Fdra-infiniband-driver?inv=1&invt=Abs5-A&project=k8s-staging-images
[container-staging]: https://console.cloud.google.com/artifacts/docker/k8s-staging-images/us-central1/dra-infiniband-driver/dra-infiniband-driver?inv=1&invt=Abs5-A&project=k8s-staging-images
[image list]: https://github.com/kubernetes/k8s.io/blob/main/registry.k8s.io/images/k8s-staging-dra-infiniband-driver/images.yaml
[promotion job]: https://testgrid.k8s.io/sig-k8s-infra-k8sio#post-k8sio-image-promo
[releases]: https://github.com/kubernetes-sigs/dra-infiniband-driver/releases
